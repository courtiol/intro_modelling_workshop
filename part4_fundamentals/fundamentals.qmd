...
1.  formulate a null hypothesis H0 for a population:\
    $\rightarrow$ H0 is chosen in contrast to the experimental hypothesis (H1)
2.  select and measure a sample
3.  question: is the sample compatible with H0?
...
3.  select a sample
4.  hypothesise a distribution of $f$ under H0; e.g. $f$ is assumed to be normally distributed (bell-shaped)
5.  according to H0, mean($f$) = 0; SD($f$) is derived from the selected sample
...
1.  null hypothesis (H0: no difference) absolutely disproved
2.  p-value reflects the probability of the H0
...
5.  p-value reflects the probability that the decision "reject H0" is wrong
...
A difference between H0 and H1 is more easily detectable ("significant")
...
\bluebold{Two-sided hypothesis} (e.g. body height): H0: males = females; H1: males $\neq$ females

- H0 is rejected only if the test statistic deviates in \bluebold{either direction}
...
\bluebold{One-sided hypothesis} (e.g. body height): H0: males = females; H1: males \> females

- H0 is rejected only if the test statistic deviates in \bluebold{one direction}
- "extreme" means that the test statistic in the 5%-region (i.e. the \blueit{rejection region})\
...
#| out-height: "4cm"
#| fig-align: center
#| fig-cap: \footnotesize (H0 is assumed true)
tibble(x = 1:50,
...
- let's also imagine that we know it under H1 (although it is not possible in practice)
...
1.  calculate the test statistic presuming H1 for many randomly generated samples of size $n$
2.  determine the percentage of test results under H1, which are significant
...
$P(\mathrm{absence}|\mathrm{not\ detected}) \sim \frac{1 \times (1 - \psi)}{1 \times (1 - \psi) + (1 - p_{11})\times \psi} = \frac{1-\psi}{1-p_{11}\psi}$ if $p_{10} \sim 0$
...
- the Bayesian framework plays along well with \blueit{MCMC samplers}, a class of algorithms that can estimate distributions numerically
...
- a well-known slightly more modern algorithm (1970) directly related to this sampler is called the \blueit{Metropolis-Hastings} algorithm
...
(the denominators of $\mathrm{posterior}(\theta')$ and $\mathrm{posterior}(\theta)$ cancel out in the ratio)
...
1. discarding the \blueit{burn-in} part of the chain
...
1. discarding the \blueit{burn-in} part of the chain
2. \blueit{thinning} the chain (i.e. down-sampling to reduce autocorrelation)
...
- for specific models, specific packages make it very easy to estimate posteriors (e.g. {rstanarm}, {brms}, {ubms})
...
- van de Schoot et al. (2021) Bayesian statistics and modelling \emph{Nat Rev Methods} [doi.org/10.1038/s43586-020-00001-2](https://doi.org/10.1038/s43586-020-00001-2)
...
- accept the fact that much of the research findings is false, but contribute to build better science with your newly acquired or refreshed statistical knowledge
