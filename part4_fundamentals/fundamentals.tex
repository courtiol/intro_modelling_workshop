...
{\centering \includegraphics[width=\linewidth,height=4cm,keepaspectratio]{figures/bean_machine.png}
...
\tightlist
\item
  formulate a null hypothesis \(H_0\) for a population:\\
  \(\rightarrow\) \(H_0\) is chosen in contrast to the experimental
  hypothesis (\(H_1\))
\item
  select and measure a sample
\item
  question: is the sample compatible with \(H_0\)?
...
\item
  select a sample
\item
  hypothesise a distribution of \(f\) under \(H_0\); e.g.~\(f\) is
  assumed to be normally distributed (bell-shaped)
\item
  according to \(H_0\), mean(\(f\)) = 0; SD(\(f\)) is derived from the
  selected sample
\item
...
\tightlist
\item
  null hypothesis (\(H_0\): no difference) absolutely disproved
\item
  p-value reflects the probability of the \(H_0\)
\item
...
\item
  ...
  (different means)
\item
  p-value reflects the probability that the decision ``reject \(H_0\)''
  is wrong
\item
...
A difference between \(H_0\) and \(H_1\) is more easily detectable
(``significant'')
...
To evaluate a difference between \(H_0\) and \(H_1\) one has to
consider:
...
\bluebold{Two-sided hypothesis} (e.g.~body height): \(H_0\): males =
females; \(H_1\): males \(\neq\) females

\begin{itemize}
\tightlist
\item
  \(H_0\) is rejected only if the test statistic deviates in
...
\bluebold{One-sided hypothesis} (e.g.~body height): \(H_0\): males =
females; \(H_1\): males \textgreater{} females
...
\caption{\footnotesize (\(H_0\) is assumed true)}
...
{\centering \includegraphics[width=\linewidth,height=7cm,keepaspectratio]{figures/xkcd_significance_wide.png}
...
\item
  let's also imagine that we know it under \(H_1\) (although it is not
...
\tightlist
\item
  calculate the test statistic presuming \(H_1\) for many randomly
  generated samples of size \(n\)
\item
  determine the percentage of test results under \(H_1\), which are
  significant
\end{enumerate}
...
\textbf{NB:} P(\(H_1\) \textbar{} reject \(H_0\)) \(\neq\) P(reject
\(H_0\) \textbar{} \(H_1\))
\end{frame}
...
\(P(\mathrm{absence}\mid\mathrm{not\ detected}) \sim \frac{1 \times (1 - \psi)}{1 \times (1 - \psi) + (1 - p_{11})\times \psi} = \frac{1-\psi}{1-p_{11}\psi}\)
if \(p_{10} \sim 0\)
\end{frame}
...
\tightlist
\item
  \blueit{MCMC samplers} are a class of algorithms that can estimate
  distributions numerically \& they are the tool that unleashed the
  potential of using Bayesian statistics in practice (kickstarted by the
  seminal paper Gelfand \& Smith 1990)
\item
...
\item
  several other MCMC algorithms directly relate to this sampler
...
\NormalTok{my\_prior\_mu }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(mu, }\AttributeTok{log =} \ConstantTok{TRUE}\NormalTok{) \{}
  ...
  \FunctionTok{dnorm}\NormalTok{(}\AttributeTok{x =}\NormalTok{ mu, }\AttributeTok{mean =} \DecValTok{5}\NormalTok{, }\AttributeTok{sd =} \DecValTok{5}\NormalTok{, }\AttributeTok{log =}\NormalTok{ log)}
\NormalTok{\}}
...
\NormalTok{my\_prior\_sd }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(sd, }\AttributeTok{log =} \ConstantTok{TRUE}\NormalTok{) \{}
  ...
  \FunctionTok{dunif}\NormalTok{(}\AttributeTok{x =}\NormalTok{ sd, }\AttributeTok{min =} \DecValTok{0}\NormalTok{, }\AttributeTok{max =} \DecValTok{15}\NormalTok{, }\AttributeTok{log =}\NormalTok{ log)}
\NormalTok{\}}

\NormalTok{my\_logPrior }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(param) \{}
...
consider them independent, which simplifies things (no need from
multivariate prior distribution) and is valid here
\end{column}
\end{columns}
\end{frame}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{my\_logPosterior }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(param) \{}
    \CommentTok{\# adding logs = multiplying originals}
    \FunctionTok{my\_logLik}\NormalTok{(param) }\SpecialCharTok{+} \FunctionTok{my\_logPrior}\NormalTok{(param)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{my\_logPosterior}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{, }\FloatTok{0.5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{my\_logPosterior}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{, }\FloatTok{0.51}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{        proba\_accept }\OtherTok{\textless{}{-}} \FunctionTok{exp}\NormalTok{(}\FunctionTok{my\_logPosterior}\NormalTok{(param\_candidate) }\SpecialCharTok{{-}}
                              \FunctionTok{my\_logPosterior}\NormalTok{(chain[i, ])) }\CommentTok{\# = ratio of post. densities}
...
\tightlist
\item
  we need to discard the \blueit{warmup} (= \blueit{burn-in}) part of
  the chain
\end{itemize}
...
\item
  ...
  the chain
\item
  chains are strongly \blueit{autocorrelated}, so \blueit{thinning} the
  chain (i.e.~down-sampling to reduce autocorrelation) is possible (to
  reduce memory footprint), but is usually not necessary (I still do it
...
\begin{center}
\includegraphics[width=\linewidth,height=3cm,keepaspectratio]{fundamentals_files/figure-beamer/unnamed-chunk-77-1.pdf}
\end{center}
...
\begin{enumerate}
\tightlist
\item define a prior distribution
\item initialise a \blueit{chain} (a vector) with a starting parameter value $\theta$
\item propose a new parameter value $\theta'$ by adding a small random jump to the current $\theta$
\item accept $\theta'$ if it increases the posterior density, otherwise accept it with a probability defined by the ratio of the posterior densities (i.e. $\dfrac{\mathrm{posterior}(\theta')}{\mathrm{posterior}(\theta)}$)
\item if $\theta'$ is accepted, add it to the chain, otherwise add $\theta$ again
\item repeat steps 3--5 until the chain reaches the target length
\item drop the beginning of the chain (i.e. the \blueit{burn-in}) to remove the influence of starting values
\item the remaining values in the chain are \blueit{samples from the posterior distribution}
\end{enumerate}
\end{small}
\end{frame}
...
\item
  ...
  distributions
\item
  it works because the algorithm need not computing the normalising
  constant of the posterior (the denominators of
  \(\mathrm{posterior}(\theta')\) \& \(\mathrm{posterior}(\theta)\)
  cancel out in the ratio)
\item
  working around maximising the likelihood largely explains the success
  of Bayesian statistics in Ecology -- a field where observational data
  typically call for complex models (e.g.~\blueit{state space models},
  where some processes cannot be directly observed, as in
  \blueit{occupancy models})
\end{itemize}
\end{frame}
...
\item
  ...
  posteriors (e.g.~\{brms\}, \{rstanarm\}, \{ubms\})
\item
...
\NormalTok{code }\OtherTok{\textless{}{-}} \FunctionTok{nimbleCode}\NormalTok{(\{ }
  \ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_len}\NormalTok{(N)) \{ }\CommentTok{\# defining model}
...
\NormalTok{  mu }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dnorm}\NormalTok{(}\AttributeTok{mean =} \DecValTok{5}\NormalTok{, }\AttributeTok{sd =} \DecValTok{5}\NormalTok{)}
\NormalTok{  sigma }\SpecialCharTok{\textasciitilde{}} \FunctionTok{dunif}\NormalTok{(}\AttributeTok{min =} \DecValTok{0}\NormalTok{, }\AttributeTok{max =} \DecValTok{15}\NormalTok{)}
\NormalTok{\})}
...
\NormalTok{results\_nimb }\OtherTok{\textless{}{-}} \FunctionTok{nimbleMCMC}\NormalTok{(}\AttributeTok{code =}\NormalTok{ code,}
                           ...
                           \AttributeTok{nchains =} \DecValTok{4}\NormalTok{,}
                           \AttributeTok{setSeed =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{, }\CommentTok{\# one seed per chain}
...
\item
  ...
  diagnostics)
\item
  when the sample size is large, Bayesian vs frequentist fits should
  converge towards the same results since the influence of the prior
  decreases with \(n\)
\end{itemize}
\end{frame}
...
\begin{frame}{Bayesian statistics --- Bayesian vs frequentist
statistics}
\protect\phantomsection\label{bayesian-statistics-bayesian-vs-frequentist-statistics-1}
Marc Kéry \& Andy Royle, ``Applied Hierarchical Modeling in Ecology''\\
\href{https://eesc.usgs.gov/mbr/pubanalysis/keryroylebook/index.html}{eesc.usgs.gov/mbr/pubanalysis/keryroylebook/index.html}

\vspace{1em}

\begin{quote}
\footnotesize ``Are all HMs Bayesian models? No, not at all! Although in
practice HMs are perhaps more often fit using Bayesian than non-Bayesian
methods, there is nothing intrinsically Bayesian about an HM (Cressie
and Wikle, 2011). In our books we use both likelihood and Bayesian
methods to fit HMs. Indeed, we are a little weary of what we perceive as
completely unnecessary trench wars between anti-Bayesians and Bayesians
\ldots{} and we are very glad to note that they have ebbed down quite a
bit during the last 2 decades. In such discussions, from the perspective
of an applied statistical modeller, minute differences between the two
approaches tend to be blown up completely out of proportion. We think
that in practice the commonalities between non-Bayesian maximum
likelihood and Bayesian inference are far, far greater than are their
differences. Most of all, both likelihood and Bayesian inference are
firmly rooted in the concept of a parametric statistical model, and in
most practical applications the two will yield inferences that are
numerically nearly indistinguishable.''
\end{quote}
\end{frame}
...
\tightlist
\item
  accept the fact that much of the research findings are false, but
  contribute to building better science with your newly acquired or
