...
- interpreting correctly model parameters
...
- linear models encompass the classic linear regression which forms a straight line, but also many other models (e.g. polynomials are linear and yet do not form straight lines)
...
fit_alien_lm_brms <- brm(size ~ humans_eaten, data = Alien, seed = 123,
                         file = "cache_brms/fit_alien_lm_brms") # storing the compiled model 
                                                                # makes it quick to rerun
                                                                # but dangerous if something changes
mcmc_plot(fit_alien_lm_brms, type = "trace") # many diagnostic plots can be produced
```
...
expand_grid(prior, humans_eaten = humans_eaten_possible) |>
  mutate(size = intercept[sim] + slope[sim]*humans_eaten,
         resp = size + rnorm(n = length(sim), 0, sigma[sim])) -> prior_pred
...
posterior1 <- tibble(
  ...
  error = rnorm(n = length(fit_alien_lm_nimb_d[,1 , 1]),
                sd = fit_alien_lm_nimb_d[, 1, "sigma", drop = TRUE]))

expand_grid(posterior1, humans_eaten = Alien$humans_eaten) |> 
  mutate(pred = intercept + slope * humans_eaten + error) -> pred1
...
- tests, SE, confidence intervals and prediction intervals, all focus on such coefficients
...
expand_grid(prior,
            flipper_len_z = flipper_len_z_possible) |>
  mutate(body_mass_z = intercept[sim] + slope[sim]*flipper_len_z
         ) -> prior_pred
...
fit_alien_glm_brms <- brm(departed ~ humans_eaten, data = Alien2,
                          ...
                          seed = 123,
                          iter = 10000,
                          file = "cache_brms/fit_alien_glm_brms") # to store the compiled model (quick to rerun)
...
- on average, \blueit{the log number of territories} is `{r} coef(fit_bird_glm)[["(Intercept)"]]` when the median elevation is 0m and the number of visit is 0
- on average, every additional meter in median elevation increases \blueit{the log number of territories} by `{r} coef(fit_bird_glm)[["median_elevation"]]`
- on average, every additional visit increases \blueit{the log number of territories} by `{r} coef(fit_bird_glm)[["n_visits"]]`
:::
:::
...
- on average, every additional human eaten increases \blueit{the odds of departure} $e^{\text{slope}}=$ `{r} exp(coef(fit_alien_glm)[["humans_eaten"]])` \blueit{fold}
:::
:::
...
- on average, every additional meter in median elevation increases the number of territories by $100 \times (e^{\text{slope}}-1)=$ `{r} 100*(exp(coef(fit_bird_glm)[["median_elevation"]])-1)`\blueit{\%}, i.e. decreases it by $100\times (1 - e^{\text{slope}})=$ `{r} 100*(1-exp(coef(fit_bird_glm)[["median_elevation"]]))`\blueit{\%}

- on average, every additional 1000 meter in median elevation increases the number of territories by $100 \times (e^{ \text{slope}\times1000}-1)=$ `{r} 100*(exp(coef(fit_bird_glm)[["median_elevation"]]*1000)-1)`\blueit{\%}, i.e. decreases it by $100\times (1 - e^{\text{slope}\times1000})=$ `{r} 100*(1-exp(coef(fit_bird_glm)[["median_elevation"]]*1000))`\blueit{\%}
:::
:::
