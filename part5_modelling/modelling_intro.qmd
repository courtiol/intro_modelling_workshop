...
Demonstration about exponential of estimates as odds ratio

```{r}
#| echo: true
p1 <- predict(fit_alien_glm, newdata = tibble(humans_eaten = 1), type = "response")
p2 <- predict(fit_alien_glm, newdata = tibble(humans_eaten = 2), type = "response")
p5 <- predict(fit_alien_glm, newdata = tibble(humans_eaten = 5), type = "response")
p6 <- predict(fit_alien_glm, newdata = tibble(humans_eaten = 6), type = "response")
...
(p2/(1 - p2)) / (p1/(1 - p1)) # odds ratio for 2 humans eaten compared to 1 eaten
(p6/(1 - p6)) / (p5/(1 - p5)) # odds ratio for 6 humans eaten compared to 5 eaten
```
...
exp(coef(fit_alien_glm)["humans_eaten"])
```
...
- on average, every additional meter in median elevation changes the number of territories by $100 \times (e^{\text{slope}}-1)=$ `{r} signif( 100*(exp(coef(fit_bird_glm)[["median_elevation"]])-1), 3)`\blueit{\%}

- on average, every additional 1000 meters in median elevation increases the number of territories by $100 \times (e^{ \text{slope}\times1000}-1)=$ `{r} signif(100*(exp(coef(fit_bird_glm)[["median_elevation"]]*1000)-1), 3)`\blueit{\%}
:::
:::
...
- \blueit{negative binomial model} (e.g. {spaMM}, {glmmTMB}, {brms}, {nimble})
- \blueit{zero-augmented models}
  - zero-inflated models  (e.g. {glmmTMB}, {brms}, {nimble})
  - hurdle models (e.g. {spaMM}, {glmmTMB}, {brms}, {nimble})
- negative binomial zero-augmented models (e.g. {glmmTMB}, {brms}, {nimble})
