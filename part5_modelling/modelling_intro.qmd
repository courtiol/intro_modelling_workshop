...
Let's create a single clean dataset with all the variables we will need for a models
...
4. does body mass increases with flipper length in males?
...
Fitting GLM with {brm} is slightly different since it uses a specific family for binary data\footnote{the Bernoulli distribution is a special case of the binomial data when there is only 1 trial}
...
... but again we need to make sure the priors are sensible and here it is even more difficult to foresee
...
expand_grid(sim = seq_len(n_sims2),
  ...
  mutate(prob = 1/(1 + plogis(prior_intercept2[sim] + 
...
fit_alien_glm_nimb <- nimbleMCMC(code = code_glm,
                                 ...
                                 nburnin = 4000)
summary(as_draws(fit_alien_glm_nimb))
```
...
## GLM --- model fitting: interlude on logit
...
- it is the inverse of the standard logistic distribution, and is defined as: $\text{logit}(p) = \text{log}\frac{p}{1-p}$
...
- it is the scale in which the linear predictor of the current GLM is expressed since the logit is allowed to vary between $-\infty$ and $\infty$ while $p$ is constrained to vary between -1 and 1
