...
Computing and plotting prediction can be difficult, but the package {modelbased} from the {easystats} universe ([easystats.github.io/easystats](https://easystats.github.io/easystats)) makes this easy for many types of models
...
territories |> 
    ...
    summarise(territories_top6 = sum(territories_total),
...
- on average, every additional meter in median elevation increases \blueit{the log number of territories} by `{r} signif(coef(fit_bird_glm)[["median_elevation"]], 3)`
...
F-test testing H0 that the variance explained by the 2 LMs is the same
```{r}
#| echo: true
fit_alien_lm      <- lm(size ~ humans_eaten, data = Alien)
...
\bluebold{Interpretation:} the observed data are `{r} round(BF_lm2$bf, 1)` times more likely under our first LM than under the null model, which represents a strong support in favour of our model
...
1. there is an `{r} round(100*proba_incl, 1)`% posterior probability that the effect of the number of humans eaten on the size of aliens is present, versus a `{r} round(100*(1- proba_incl), 1)`% posterior probability that the data are equally consistent with the null model
...
t-tests testing H0 that the parameter estimates are 0
```{r}
#| echo: true
...
- this is directly provided by the summary tables we have seen all along
...
using credibility intervals for testing H0 that the parameter estimates are 0
```{r}
#| echo: true
...
- this is again provided by the summary tables we have seen earlier

## Tests --- conclusion

- testing parameters directly is much easier but it increases the risk of false positives (this is why I recommended earlier the testing order going from overall to specific)
- in Bayesian statistics, the Bayes factor is appealing since it potentially applies to all possible testing situations, however it is very dependent on priors and thus on internal scaling of variables by software, which makes it a measure difficult to compare across studies, casting doubt on the usual "scale" of strength of evidence it is associated with
...
# A simple habitat occupancy model
