...
Let's enter \bluebold{God(dess) mode} $\rightarrow$ we pretend we know how the world generate data
...
We can thus generate the size of 6 aliens that have eaten between 1 and 3 humans following E.L.'s recipe:

```{r}
#| label: alien data
#| echo: true
set.seed(123L)
Alien <- tibble(humans_eaten = rep(1:3, each = 2), intercept = 50, slope = 1.5,
                error = rnorm(n = 6, mean = 0, sd = 2),
...
#| echo: true
#| eval: false
size <- rnorm(n = 6, mean = intercept + slope*humans_eaten, sd = 2)
```
...
Let's first use {brms}, which is simpler than {nimble} but handle a smaller set of models:
```{r}
...
library(brms) # Bayesian regression models using stan
fit_alien_lm_brms <- brm(size ~ humans_eaten, data = Alien,
                         file = "cache_brms/fit_alien_lm_bayes") # to store the compiled model (quicker to rerun)
summary(fit_alien_lm_brms)
```
...
- despite large uncertainties ($N=6$) the methods are rather consistent and not too far off
- in `lm()` $\hat{\sigma}$ is usually not considered as a true parameter, it is deduced once the intercept and slope(s) have been fitted (and behind the scene I struggled to compute the SE and CI for it)
