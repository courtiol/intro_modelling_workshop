...
an approximate description of the \blueit{data generating process}, i.e. an approximation of how the world generate data
...
- perform prior and posterior predictive checks during Bayesian analyses
...
- much of difficulties people encounter when dealing with more complex models (e.g. Generalised Linear Models, Linear Mixed-effects Models, Hierarchical Models) is often caused by not having sufficiently understood the LM
...
- when parameters are not know but estimated they should wear a hat (e.g. $\hat{\beta_0}$) and $\epsilon_i$ (also sometimes called $\varepsilon_i$) usually becomes $e_i$ or $r_i$
...
fit_alien_lm_brms <- brm(size ~ humans_eaten, data = Alien, seed = 123,
                         file = "cache_brms/fit_alien_lm_bmrs") # storing the compiled model 
...
expand_grid(prior, humans_eaten = humans_eaten_possible) |>
  mutate(size = intercept[sim] + slope[sim]*humans_eaten,
         resp = size + sigma[sim]) -> prior_pred

ggplot(prior_pred) +
  aes(x = humans_eaten, y = size) +
...
expand_grid(posterior1, humans_eaten = Alien$humans_eaten) |> 
  mutate(pred = intercept + slope * humans_eaten) -> pred1
...
\begin{scriptsize}
\begin{itemize}
\item this looks OK: the posterior predictions are much more concentrated than the prior ones, and close to the observations although the number of observations is a little to low to judge properly
\end{itemize}
\end{scriptsize}
:::
:::
...
\begin{footnotesize}
\begin{itemize}
\item<2-> we never obtained 50 (intercept), 1.5 (slope), or 2 (sigma) because a sample is unlikely to reflect perfectly the population-level values; only N=$\infty$ or talking to the goddess could get you those
\item<3-> despite the small sample size (`{r} paste0("N=", N)`) the estimates are in agreement and all intervals include ground truth values
\end{itemize}
\pause
\vspace{1em}
...
Let's create a single clean dataset with all the variables we will need for everything
...
- the \blueit{intercept} remains the prediction when all regressors are at 0, but `flipper_len = 0` meant a flipper length of 0 mm, while `flipper_len_z = 0` means an average flipper length; so here the intercept corresponds to the average body mass of a penguin that would have flippers of `{r} round(mean(penguins_clean$flipper_len), 1)` mm  (i.e. no longer nonsensical)
...
- the \blueit{slope} remains the change in predicted values when its associated regressor increases by 1 unit, but here 1 unit is not longer 1 mm but 1 SD in flipper length; so here it means that the model predicts that penguins with flippers 1 SD longer than others (`{r} round(sd(penguins_clean$flipper_len), 1)` mm) are on average `{r} round(coef(fit_flipper_z)[["flipper_len_z"]])` gr heavier than such others
...
2. the intercept is the mean scaled value of the body mass for a penguin with average flipper length, which is why it is 0 (a group of penguins with an average flipper length is expected to have an average body mass)
...
4. something like $\mathcal{N}(0, \sigma = 2)$ for both the intercept and slope priors seems appropriate (& close to {brms} default!)
...
\item<3-> by default, R choose a reference level (by default, the first in alphabetical order) and expresses other effects in relation to that reference, a method called \blueit{treatment contrast}
\end{itemize}
\end{footnotesize}
:::
:::
...
you want to compute/illustrate the relationship between `body_mass` & `flipper_len`, what to do with sex & species?

\begin{small}
\begin{itemize}
\item<2-> \bluebold{conditional predictions}: the software fixes each non-focal predictor at a specific value (e.g. mean for quantitative variables, most frequent category for the qualitative variables) $\rightarrow$ easiest, but choice of reference can be arbitrary and computation even slightly wrong in more complex models involving non-linear transformation(s) (due to \blueit{Jensen's inequality})
\item<3-> \bluebold{marginal predictions}: the software computes predictions for different values/levels of the focal and non-focal predictors and then averages those predictions across the non-focal predictors $\rightarrow$ often better, but more complex since different alternatives exist to compute the \blueit{reference grid} (depending on whether one wants to capture or erase the actual or structural correlations between predictors\footnote{structural correlations are caused by an unbalanced sampling design})
\end{itemize}
\end{small}
...
\bluebold{Marginal predictions} of the effect of `flipper_len` on `body_mass` ("average")
...
```
\footnotesize \textbf{NB:} here, the marginal prediction is the average of the predicted values for all sampled penguins with the same flipper length and which my differ in their sex or species $\rightarrow$ preserves correlations between predictors & preserves the distributions of non-focal predictors (i.e. sex & species sampled more weigh more)
:::
:::
...
\bluebold{Marginal predictions} of the effect of `flipper_len` on `body_mass` ("typical")
...
\bluebold{Marginal predictions} of the effect of `flipper_len` on `body_mass` ("population")
...
- proportion (e.g. the hatching success in a nest), especially if they can get close to 0 or 1
- count data (e.g. the number of species caught on camera), especially if counts can be low
- time to event data (e.g. time to dispersion), especially if times can be short

For these situations GLM are often the first choice!
...
We can thus generate data on 35 aliens following E.L. recipe:
...
Alien2 <- tibble(humans_eaten = rep(0:6, each = 5),
                 prob_departure = 1/(1 + exp(-(-2.197 + 1.5*humans_eaten))),
...
```{r}
#| label: alien glm fake
#| echo: true
#| eval: false
fit_alien_glm <- glm(departed ~ humans_eaten, data = Alien2, family = binomial(link = "logit"))
summary(fit_alien_glm) # or coef(fit_alien_glm) for a focus on coefficients only
```

```{r}
#| label: alien glm real
#| echo: false
#| size: scriptsize
fit_alien_glm <- glm(departed ~ humans_eaten, data = Alien2,
                     family = binomial(link = "logit"))
summary(fit_alien_glm)
```

## GLM --- model fitting: Bayesian approach {brms}
...
fit_alien_glm_brms <- brm(departed ~ humans_eaten, data = Alien2,
                          family = bernoulli(link = "logit"),
                          seed = 123,
                          file = "cache_brms/fit_alien_glm_bmrs") # to store the compiled model (quick to rerun)
...
code_glm <- nimbleCode({ 
  ...
  for (i in seq_len(N)) {
      departed[i] ~ dbinom(size = 1, prob = plogis(intercept + slope*humans_eaten[i]))
  }
  # prior
...
constants <- list(N = nrow(Alien2),
                  humans_eaten = Alien2$humans_eaten)

inits <- list(intercept = mean(Alien2$departed), slope = 0)
```

## GLM --- model fitting: Bayesian approach (prior sanity check)
...
set.seed(123)
n_sims2 <- 100
prior_intercept2 <- rnorm(n = n_sims2, 
                          mean = 0, sd = 2)
prior_slope2 <- rnorm(n = n_sims2,
                      mean = 0, sd = 2)

expand_grid(sim = seq_len(n_sims2),
            humans_eaten = seq(0, 6, by = 0.1)) |> 
  mutate(prob = 1/(1 + exp(-
     (prior_intercept2[sim] +
      prior_slope2[sim]*humans_eaten)
     ))) -> prior_pred2
```
:::
::: {.column width="40%"}
